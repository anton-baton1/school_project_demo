...
from errors import LenPasswordError, BigAlphaPasswordError, SymbolPasswordError, SmallAlphaPasswordError, \
    DigitPasswordError, AlphaPasswordError, SymbolLoginError, ExistLoginError
from registration_page_ui import Ui_Form
...
class RegistrationForm(QWidget, Ui_Form):
    ...
    def registration(self):
        ...
        try:
            if self.data_base.cursor().execute("SELECT login FROM users WHERE login = ?", (login,)).fetchone():
                raise ExistLoginError
            for i in login:
            ...
            self.data_base.cursor().execute("INSERT INTO users(login, password) VALUES(?, ?)",
                                            (login, password,))
        ...
        except ExistLoginError:
            self.error_password.setText("")
            self.error_login.setText("Данный логин уже занят")
        except SymbolLoginError:
            self.error_password.setText("")
            self.error_login.setText("Логин может содержать только буквы, цифры и символы '_-@'")
